...
# Own tests 
def test_invalid_when_multiple_operators():
    df = pd.DataFrame([[1, 2]], columns=["a", "b"])
    result = add_virtual_column(df, "a+b-c", "c")
    assert result.empty

def test_invalid_when_no_operator():
    df = pd.DataFrame([[1, 2]], columns=["a", "b"])
    result = add_virtual_column(df, "ab", "c")
    assert result.empty

def test_invalid_when_column_not_exists():
    df = pd.DataFrame([[1, 2]], columns=["a", "b"])
    result = add_virtual_column(df, "a+c", "d")
    assert result.empty

def test_invalid_when_empty_role():
    df = pd.DataFrame([[1, 2]], columns=["a", "b"])
    result = add_virtual_column(df, "   ", "c")
    assert result.empty

def test_invalid_when_df_not_dataframe():
    result = add_virtual_column("not a df", "a+b", "c")
    assert result.empty
